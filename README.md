...
## Run locally 
- Install [Docker Desktop](https://www.docker.com/products/docker-desktop/) and [Docker Compose plugin](https://docs.docker.com/compose/install/) *(Docker Compose is available by default in Docker Desktop)*
- Clone the repository using `git` \
	`git clone https://github.com/kumarsandeep567/twtr.git`
- To start the application, navigate to the project folder and run \
	`docker-compose up -d` *(`-d` is for running in detached mode)* 
- To stop the application (and delete the containers), run \
	`docker-compose down`


## Build instructions (on Amazon Web Services)
...
> [!Caution]
> Docker users, you might get rate limited for too many pull requests.
> Be aware of your pull request limit at [Docker Hub rate limit](https://docs.docker.com/docker-hub/download-rate-limit/)
---
